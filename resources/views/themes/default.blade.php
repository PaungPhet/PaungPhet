...
<head>
    ...
    <style>
        ...
        .glass-panel {
            background: rgba(255, 255, 255, 0.75); /* Slightly more transparency */
...
<main class="w-full min-h-screen relative">
    ...
    <div class="relative w-full h-[85vh] flex items-center justify-center overflow-hidden">
        ...
        <div class="relative z-10 text-center text-white px-4 animate-fade-in-up">
            ...
            <h1 class="font-script text-5xl md:text-6xl lg:text-7xl mb-6 drop-shadow-md leading-tight text-white/95">
                {{ $wedding->partner_one }} <br class="md:hidden"> <span
                    class="mx-2 text-4xl md:text-6xl text-pink-200">&</span> <br
                    class="md:hidden"> {{ $wedding->partner_two }}
            </h1>

            <p class="uppercase tracking-[0.4em] text-xs md:text-sm mb-4 opacity-90 font-light">{{ __('theme/default.event_subtitle') }}</p>

            <div class="mt-8 glass-panel text-pink-500 inline-block px-8 py-3 rounded-[3rem] shadow-lg shadow-pink-200/20">
                <p class="text-xl md:text-2xl font-semibold tracking-wide">{{ $guest->name }}</p>
                <p class="mt-1 text-[10px] uppercase tracking-widest text-gray-700 font-medium">{{ __('theme/default.invitee_subtitle') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-3xl mx-auto -mt-20 relative z-20 px-4 pb-20 space-y-10">
        ...
        <div class="glass-panel p-10 rounded-[2.5rem] shadow-xl shadow-pink-100/60 ring-1 ring-white/60">
            <h2 class="font-script text-4xl text-center text-pink-400 mb-6">{{ __('theme/default.content_title') }}</h2>
            <div class="prose prose-pink mx-auto text-center text-slate-600 leading-loose font-light">
                {!! $wedding->content_renderer !!}
            </div>
        </div>

        <div class="p-8 rounded-[2.5rem] shadow-lg shadow-pink-100/50 bg-white/80 backdrop-blur-sm text-center border border-white">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 divide-y md:divide-y-0 md:divide-x divide-pink-100">
                ...
                <div class="flex flex-col items-center p-2 pt-8 md:pt-2">
                    ...
                    <span class="text-sm font-medium text-slate-600 tracking-wide">{{ $wedding->event_time ?? '00:00' }}</span>
                </div>
        ...
        @if($wedding->images->count() > 0)
            <div class="bg-white p-5 rounded-[2.5rem] shadow-xl shadow-pink-100/50 overflow-hidden border border-pink-50/50">
                <h2 class="font-script text-3xl text-center text-pink-400 mb-6 mt-2">{{ __('theme/default.prewedding_images_title') }}</h2>
            ...
            <div class="bg-white p-10 rounded-[2.5rem] shadow-xl shadow-pink-100/50 text-center border border-pink-50">
                ...
                <div class="flex flex-col md:flex-row gap-8 items-center justify-center">
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <span class="text-[10px] text-pink-300 uppercase tracking-widest font-bold">{{ __('theme/default.address_map_scan') }}</span>
                    </div>
        ...
        @if($guest->is_notable)
            @if($guest->note)
                <div class="bg-green-50/80 backdrop-blur-sm text-green-600 p-6 rounded-[2rem] text-center border border-green-100 mb-4 shadow-sm">
                    {{ __('theme/default.note_sent') }}
                </div>
            @else
                <div class="glass-panel p-10 rounded-[2.5rem] shadow-xl shadow-pink-100/40 border border-pink-100">
                    <h2 class="font-script text-3xl text-center text-pink-400 mb-2">{{ __('theme/default.note_title') }}</h2>
                    <p class="text-center mt-2 text-slate-400 mb-8 font-light text-sm">{{ __('theme/default.note_subtitle') }}</p>
                    ...
                    @elseif(\Illuminate\Support\Str::of($guest->note)->trim()->isNotEmpty())
                        <div class="bg-pink-50/50 p-8 rounded-[2rem] text-center italic text-pink-800 border border-pink-100">
                            "{{ $guest->note }}"
                            <div class="mt-4 text-xs text-pink-300 font-bold uppercase not-italic tracking-wider">{{ __('theme/default.note_sent') }}</div>
                        </div>
                    @else
                        <form method="POST"
